...
export interface EnhancedSOSRequest {
  ...
  suggestedEscalation: 'local_police' | 'ranger' | 'medical' | 'fire_department';
}

class SOSAIService {
  private riskNegativeKeywords = [
    'unsafe', 'robbery', 'bad lighting', 'isolated', 'unsafe at night',
    'dangerous', 'avoid', 'crime', 'theft', 'mugging', 'scam',
    'dark', 'empty', 'sketchy', 'rough area', 'not safe'
  ];
  ...
  async analyzeRiskProfile(touristId: string): Promise<TouristRiskProfile> {
    try {
      ...
      const factorScore = factors.reduce((sum, factor) => 
        sum + (factor.score * factor.weight), 0) / factors.length || 0;
  ...
  async getSOSAnalytics(touristId: string): Promise<SOSAnalytics> {
    try {
      ...
      const alertsLast24h = alerts?.filter(
        alert => new Date(alert.created_at) >= last24h
      ).length || 0;

      const alertsLast7d = alerts?.filter(
        alert => new Date(alert.created_at) >= last7d
      ).length || 0;

      // Analyze time patterns
      const timePatterns = alerts?.map(alert => {
  ...
  async createEnhancedSOS(request: Omit<EnhancedSOSRequest, 'riskScore' | 'priority' | 'suggestedEscalation'>): Promise<{
    success: boolean;
    sosId?: string;
    ...
    try {
      ...
      if (riskProfile.factors.some(f => f.type === 'frequency' && f.score > 80)) {
        riskScore += 20;
      }
      ...
      let suggestedEscalation: EnhancedSOSRequest['suggestedEscalation'] = 'local_police';
      ...
      const sosData = {
        ...
        blockchain_hash: `sos_${request.touristId}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
      };
  ...
  async checkLocationRisk(latitude: number, longitude: number): Promise<{
    isRisky: boolean;
    riskLevel: 'LOW' | 'MEDIUM' | 'HIGH';
    warnings: string[];
    source: string;
    ...
    try {
      ...
      const mockGoogleResponse = await this.simulateGooglePlacesReview(latitude, longitude);
      ...
      if (mockGoogleResponse.reviews) {
        for (const review of mockGoogleResponse.reviews) {
          ...
          const foundKeywords = this.riskNegativeKeywords.filter(
            keyword => reviewText.includes(keyword)
          );
  ...
  private async triggerAlertNotifications(sosRecord: Record<string, unknown>, priority: string, riskProfile: TouristRiskProfile) {
    try {
  ...
  getDeviceContext(): {
    ...
    return {
      ...
      networkStrength: networkInfo ? 
        Math.round(((networkInfo as Record<string, number>).downlink || 5) / 10 * 100) : 80,
      deviceInfo: {
        userAgent: navigator.userAgent,
        isOnline: navigator.onLine,
        connectionType: (networkInfo as Record<string, string>)?.effectiveType || 'unknown'
      }
    };
  }
}
