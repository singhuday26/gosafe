...
export const GeoFenceEditor: React.FC<GeoFenceEditorProps> = ({
  ...
  }, [mapboxToken, center, zoom, renderGeoFences]);
  ...
  }, [geoFences, isLoading, renderGeoFences]);
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Existing GeoFences ({geoFences.length})</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
            {geoFences.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                No geofences created yet. Draw a polygon on the map above to get
                started.
              </p>
            ) : (
              geoFences.map((fence) => (
